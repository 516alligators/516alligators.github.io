...
twenty <- 
  ...
  dplyr::select(id, text = `full_text`, year)
...
prp_twenty_most_common %>%  dplyr::select("Word",`2020 proportion`) %>% arrange(-`2020 proportion`) -> prp_twenty_most_common
...
prp_twentyone_most_common %>%  dplyr::select("Word",`2021 proportion`) %>% arrange(-`2021 proportion`) -> prp_twentyone_most_common
...
twenty_afinn %>% dplyr::select(word, value) %>% distinct() -> twenty_afinn2
...
twentyone_afinn %>% dplyr::select(word, value) %>% distinct() -> twentyone_afinn2
...
for (x in 1:10) {
  twenty_termslist <- 
    append(twenty_termslist, twenty_terms %>% dplyr::select(x) %>% 
             unname() %>% unlist())
}
...
for (x in 1:10) {
  twentyone_termslist <- 
    append(twentyone_termslist, twentyone_terms %>% dplyr::select(x) %>% 
             unname() %>% unlist())
}
...
twenty_rts_afinn %>% dplyr::select(word, value) %>% distinct() -> twenty_rts_afinn2
...
twentyone_rts_afinn %>% dplyr::select(word, value) %>% distinct() -> twentyone_rts_afinn2
...
for_decisiontree_twenty <-
  twenty_rts_feature_selection %>% dplyr::select(-1,-2,-3,-4,-5,-6,-`id`) %>% drop_na()
...
for_decisiontree_twentyone <-
  twentyone_rts_feature_selection %>% dplyr::select(-1,-2,-3,-4,-5,-6,-`id`) %>% drop_na()
...
```{r}
hashtags20 <-
  kbl(head(tstat_freq_twenty %>% as.data.frame() %>% dplyr::select(1,2), 20, digits = 2), booktabs = T) %>% 
  kable_styling()

hashtags21 <-
  kbl(head(tstat_freq_twentyone %>% as.data.frame() %>% dplyr::select(1,2), 20, digits = 2), booktabs = T) %>% 
  kable_styling()
